...
def parse_elmer_output(directory: str) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to read VC sweep file {vc_path}: {e}") from e
    ...
    try:
        leak_text = leak_path.read_text(encoding="utf-8")
    except (OSError, PermissionError) as e:
        raise RuntimeError(f"Failed to read leakage file {leak_path}: {e}") from e
    for line in leak_text.splitlines():
        line = line.strip()
        if not line:
            continue
        parts = line.split()
...
def generate_density_plot(vtu_path: str | Path, design: LoudspeakerDesign, output_path: str | Path) -> None:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to read VTU file {vtu_path}: {e}") from e
